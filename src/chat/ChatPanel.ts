...
import { readDesignDoc, addFile, updateDesignDoc, readAllFiles, Step, generateSteps, getFileHierarchy, readRelevantFiles } from '../utils';
...
export class ChatPanel implements vscode.WebviewViewProvider {
  ...
  public async resolveWebviewView(webviewView: vscode.WebviewView, context: vscode.WebviewViewResolveContext, _token: vscode.CancellationToken) {
    ...
    this.onDidReceiveMessage((message: {
      type: string;
      value?: string | number | string[] | CodeGenerationRequest | Step | Step[];
    }) => {
      switch (message.type) {
        case "readDesignDoc":
